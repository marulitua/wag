...
func loadXDP(SessionInactivityTimeoutMinutes uint64) error {
	...
	value := SessionInactivityTimeoutMinutes * 60000000000
...
func attachXDP(interf string) error {
	iface, err := net.InterfaceByName(interf)
	if err != nil {
		return fmt.Errorf("lookup network iface %q: %s", interf, err)
	}
...
func setMaps(userid [20]byte, userAcls acls.Acl) error {
...
func SetSessionInactivityTimeout(SessionInactivityTimeoutMinutes uint64) error {
...
func SetAcls(username string, userAcls acls.Acl) error {

	lock.Lock()
	defer lock.Unlock()

	return setUserAcls(username, userAcls)
}

// Non-mutex guarded internal version
func setUserAcls(username string, userAcls acls.Acl) error {

	userid := sha1.Sum([]byte(username))

	return setMaps(userid, userAcls)
}

// SetAuthroized correctly sets the timestamps for a device with internal IP address as internalAddress
func SetAuthorized(internalAddress, username string, MaxSessionLifetimeMinutes uint64) error {
	...
	deviceStruct.sessionExpiry = GetTimeStamp() + MaxSessionLifetimeMinutes*60000000000
	if MaxSessionLifetimeMinutes < 0 {
...
func GetRules() (map[string]FirewallRules, error) {
	...
	for iter.Next(&ipBytes, &deviceBytes) {
		...
		fwRule := result[hex.EncodeToString(deviceStruct.user_id[:])]
		...
		result[hex.EncodeToString(deviceStruct.user_id[:])] = fwRule
